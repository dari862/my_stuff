...
convert_2_png(){
	for FILEPATH in ${PATHLIST2[@]}; do
    	pngfile="${FILEPATH//-generated.svg/.png}"
    	inkscape --export-filename="$pngfile" "$FILEPATH"
    	rm -rf "$FILEPATH"
    	if [[ -n "$optipng" ]]; then
        	"$optipng" -o7 --quiet "$pngfile"
    	fi
	done
}

render_assets() {
	info_m "pre rendering assets"
	cd "$tmp_dir/src/gtk-2.0"
	if [[ "$OPTION_GTK2_HIDPI" == "true" ]]; then
		find "$tmp_dir/src/gtk-2.0" -type f -name "*-generated.svg" -delete
		
		find "$tmp_dir/src/gtk-2.0" -type f -name "*-generated.HIDPI.svg" -print0 | while IFS= read -r -d '' file; do
    		newname="${file//.HIDPI.svg/.svg}"
    		mv "$file" "$newname"
		done
	else
		find "$tmp_dir/src/gtk-2.0" -type f -name "*-generated.HIDPI.svg" -delete
	fi
    
    info_m "Rendering assets"
	PATHLIST2=($(find "$tmp_dir/src" -type f -name "*-generated.svg"))
