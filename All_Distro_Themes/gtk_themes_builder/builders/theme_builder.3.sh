...
render_svg() {
    ...
    # Calculate zoom factor: 96 DPI = 1x, 192 DPI = 2x
    local zoom="1"
    if [[ "$dpi" -eq 192 ]]; then
        zoom="2"
    fi
...
convert_2_png(){
	...
	render_gtk3_asset() {
        ...
        for i in $(find "${assets_dir}" -type f -name "*.svg"); do
        	i="$(basename "$i")"
    		i="${i%.*}"
			render_svg "${dpi_value}" "${i}" "${i}"
        	render_svg "${hidpi_value}" "${i}@2" "${i}"
    	done
	}
	local asset
	...
	if [[ "$OPTION_GTK2_HIDPI" == "true" ]]; then
		picked_dpi="${hidpi_value}"
	else
		picked_dpi="${dpi_value}"
	fi
    cd "$tmp_dir/src/gtk-2.0"
    assets_dir="assets"
    # Render light assets
    render_gtk2_asset "${picked_dpi}"
...
render_assets() {
    ...
    SEL_BG_new1="#$SEL_BG"
    if [[ "$MATERIA_COLOR_VARIANT" != "dark" ]]; then
    	find './src/gtk-2.0/assets-dark' -name "*.svg" -type f -delete
