...
case "$themes_style_name" in
    materia)
    	...
    	buid_materia_theme --hidpi True --target "$themes_output_dir" --output "${output_theme_name}" "${theme_file}"
        ;;
    oomox)
    	info_m "Using $themes_style_name"
    	buid_oomox_theme --hidpi True --target-dir "$themes_output_dir" --output "${output_theme_name}" "${theme_file}" --make-opts gtk320 css_cinnamon
        ;;
    arc)
    	info_m "Using $themes_style_name"
        ;;
