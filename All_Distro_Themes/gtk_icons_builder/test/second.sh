...
picknewicons(){
	post_new_icon_name="${1:-}"
	new_icon_name="$(grep "gtk-icon-theme-name=" "$HOME"/.config/gtk-3.0/settings.ini | awk -F= '{print $2}')"
	new_icon_name="${new_icon_name}1"
	sed -i "s|Net/IconThemeName .*|Net/IconThemeName  ${new_icon_name}${new_icon}|g" $HOME/.config/xsettingsd/xsettingsd.conf
	sed -i "s/gtk-icon-theme-name=.*/gtk-icon-theme-name=\"${new_icon_name}${new_icon}\"/g" $HOME/.config/gtk-2.0/gtkrc-2.0
	sed -i "s/gtk-icon-theme-name=.*/gtk-icon-theme-name=${new_icon_name}${new_icon}/g" $HOME/.config/gtk-3.0/settings.ini
...
test_panel_style(){
	...
	for file in *;do
		new_icons_list="$file|${new_icons_list}"
	done
	new_icons_list="${new_icons_list%?}"
	
	cd "${blob_path}"
	styles_list="$(grep -rEi "${new_icons_list}" | grep ":gtk_icon_name=" | awk -F'/' '{print $1}' | grep -v "owl4ce")"
