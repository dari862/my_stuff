...
rofi_command_is="rofi -no-config -no-lazy-grab -sep | -dmenu -i -p '' -theme \"$rofi_style_dir\"/styles.rasi"
# Launch Rofi
if [ "$ROFI_STYLE" = "blocks" ]
then
	MENU="$(echo " Default| Nord| Gruvbox| Adapta| Cherry" | $rofi_command_is)"
				case "$MENU" in
...
then
	MENU="$(echo " Default| Nord| Gruvbox| Dark| Cherry" | $rofi_command_is)"
				case "$MENU" in
					*Default) style-switcher.sh --default ;;
					*Nord) style-switcher.sh --nord ;;
					*Gruvbox) style-switcher.sh --gruvbox ;;
...
then
	MENU="$(echo " Black| Adapta| Dark| Red| Green| Teal| Gruvbox| Nord| Solarized| Cherry" | $rofi_command_is)"
				case "$MENU" in
...
then
	MENU="$(echo " Default| Nord| Gruvbox| Dark| Cherry| White| Black" | $rofi_command_is)"
				case "$MENU" in
					*Default) style-switcher.sh --default ;;
					*Nord) style-switcher.sh --nord ;;
					*Gruvbox) style-switcher.sh --gruvbox ;;
...
then
	...
	# Launch Rofi
	MENU="$(echo "♥ Style-1|♥ Style-2|♥ Style-3|♥ Style-4|♥ Style-5|♥ Style-6|♥ Style-7|♥ Style-8|♥ Style-9|♥ Style-10|♥ Style-11|♥ Style-12" | $rofi_command_is)"
				case "$MENU" in
					## Light Colors
	...
	# Launch Rofi
	MENU="$(echo " Budgie| Deepin| Elementary| Elementary_Dark| Gnome| KDE| KDE_Dark| Liri| Mint| Ubuntu_gnome| Ubuntu_unity| Xubuntu| Zorin" | $rofi_command_is)"
				case "$MENU" in
