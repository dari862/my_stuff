...
source ./yadtlp_configdescriptions
...
main_yad(){
	#  - name: General
	yad_opts=()
	yad_opts=(--plug=$KEY --tabnum=1 --borders=10 
	--image=/usr/share/icons/hicolor/128x128/apps/mbcc.png
	--columns=1 --form  )
    yad_opts+=( --field="TLP ENABLE:CHK" "${TLP_ENABLE}")
    [[ "$__tlp_version_" != "1.3" ]] && yad_opts+=(--field="TLP WARN LEVEL:cb" "${TLP_WARN_LEVEL}!0!1!2!3")
    yad_opts+=( --field="TLP DEFAULT MODE:cb" "${TLP_DEFAULT_MODE}!AC!BAT")
    yad_opts+=( --field="TLP PERSISTEN DEFAULT:cb" "${TLP_PERSISTENT_DEFAULT}!0!1")
    yad_opts+=( --field="TLP PS IGNORE:cb" "\"${TLP_PS_IGNORE}\"!\"AC\"$__add_usb!\"BAT\"")
	yad "${yad_opts[@]}" > $General &
	
	#  - name: Audio
	yad_opts=()
	yad_opts=(--plug=$KEY --tabnum=2 --borders=10
	--image=/usr/share/icons/hicolor/128x128/apps/mbcc.png
	--columns=1 --form)
    yad_opts+=( --field="SOUND POWER SAVE ON AC::NUM" "$SOUND_POWER_SAVE_ON_AC"!0..3600!1)
    yad_opts+=( --field="SOUND POWER SAVE ON_BAT::NUM" "$SOUND_POWER_SAVE_ON_BAT"!0..3600!1)
    yad_opts+=( --field="SOUND POWER SAVE CONTROLLER:CHK" "${SOUND_POWER_SAVE_CONTROLLER}")
	yad "${yad_opts[@]}" > $Audio &
	
	#  - name: Disks
	yad_opts=()
	yad_opts=(--plug=$KEY --tabnum=3 --borders=10
	--image=/usr/share/icons/hicolor/128x128/apps/mbcc.png
	--columns=1 --form)
    yad_opts+=( --field="DISK IDLE SECS ON AC::NUM" "$DISK_IDLE_SECS_ON_AC"!0..1000!1)
    yad_opts+=( --field="DISK IDLE SECS ON BAT::NUM" "$DISK_IDLE_SECS_ON_BAT"!0..1000!1)
    yad_opts+=( --field="MAX LOST WORK SECS ON AC::NUM" "$MAX_LOST_WORK_SECS_ON_AC"!0..10000!1)
    yad_opts+=( --field="MAX LOST WORK SECS ON BAT::NUM" "$MAX_LOST_WORK_SECS_ON_BAT"!0..10000!1)
    yad_opts+=( --field="DISK DEVICES!gtk-yes:FBTN" "bash -c _DISK_DEVICES")
    yad_opts+=( --field="DISK APM LEVEL ON AC!gtk-yes:FBTN" "bash -c _DISK_APM_LEVEL_ON_AC")
    yad_opts+=( --field="DISK APM LEVEL ON BAT!gtk-yes:FBTN" "bash -c _DISK_APM_LEVEL_ON_BAT")
	[[ "$__tlp_version_" != "1.3" ]] && yad_opts+=(--field="DISK APM CLASS DENYLIST!gtk-yes:FBTN" "bash -c _DISK_APM_CLASS_DENYLIST")
    yad_opts+=( --field="DISK SPINDOWN TIMEOUT ON AC!gtk-yes:FBTN" "bash -c _DISK_SPINDOWN_TIMEOUT_ON_AC")
    yad_opts+=( --field="DISK SPINDOWN TIMEOUT ON BAT!gtk-yes:FBTN" "bash -c _DISK_SPINDOWN_TIMEOUT_ON_BAT")
    yad_opts+=( --field="DISK IOSCHED!gtk-yes:FBTN" "bash -c _DISK_IOSCHED")
    yad_opts+=( --field="SATA LINKPWR ON AC!gtk-yes:FBTN" "bash -c _SATA_LINKPWR_ON_AC")
    yad_opts+=( --field="SATA LINKPWR ON BAT!gtk-yes:FBTN" "bash -c _SATA_LINKPWR_ON_BAT")
    yad_opts+=( --field="SATA LINKPWR DENYLIST" "$SATA_LINKPWR_DENYLIST")
    yad_opts+=( --field="AHCI RUNTIME PM ON AC:CHK" "${AHCI_RUNTIME_PM_ON_AC}")
    yad_opts+=( --field="AHCI RUNTIME PM ON BAT:CHK" "${AHCI_RUNTIME_PM_ON_BAT}")
    yad_opts+=( --field="AHCI RUNTIME PM TIMEOUT::NUM" "$AHCI_RUNTIME_PM_TIMEOUT"!0..10000!1)
    yad_opts+=( --field="BAY POWEROFF ON AC:CHK" "${BAY_POWEROFF_ON_AC}")
    yad_opts+=( --field="BAY POWEROFF ON BAT:CHK" "${BAY_POWEROFF_ON_BAT}")
    yad_opts+=( --field="BAY DEVICE" "$BAY_DEVICE")
	yad "${yad_opts[@]}" > $Disks &
	
	#  - name: Graphics
	yad_opts=()
	yad_opts=(--plug=$KEY --tabnum=4 --borders=10
	--image=/usr/share/icons/hicolor/128x128/apps/mbcc.png
	--columns=1 --form)
    yad_opts+=( --field="INTEL GPU MIN FREQ ON_AC::NUM" "$INTEL_GPU_MIN_FREQ_ON_AC"!0..5000!1)
    yad_opts+=( --field="INTEL GPU MIN FREQ ON_BAT::NUM" "$INTEL_GPU_MIN_FREQ_ON_BAT"!0..5000!1)
    yad_opts+=( --field="INTEL GPU MAX FREQ ON_AC::NUM" "$INTEL_GPU_MAX_FREQ_ON_AC"!0..5000!1)
    yad_opts+=( --field="INTEL GPU MAX FREQ ON_BAT::NUM" "$INTEL_GPU_MAX_FREQ_ON_BAT"!0..5000!1)
    yad_opts+=( --field="INTEL GPU BOOST FREQ ON_AC::NUM" "$INTEL_GPU_BOOST_FREQ_ON_AC"!0..5000!1)
    yad_opts+=( --field="INTEL GPU BOOST FREQ ON_BAT::NUM" "$INTEL_GPU_BOOST_FREQ_ON_BAT"!0..5000!1)
    yad_opts+=( --field="RADEON POWER PROFILE ON AC:cb" "${RADEON_POWER_PROFILE_ON_AC}!low!mid!high!auto!default")
    yad_opts+=( --field="RADEON POWER PROFILE ON BAT:cb" "${RADEON_POWER_PROFILE_ON_BAT}!low!mid!high!auto!default")
    yad_opts+=( --field="RADEON DPM STATE ON AC:cb" "${RADEON_DPM_STATE_ON_AC}!battery!performance")
    yad_opts+=( --field="RADEON DPM STATE ON BAT:cb" "${RADEON_DPM_STATE_ON_BAT}!battery!performance")
    yad_opts+=( --field="RADEON DPM PERF LEVEL ON AC:cb" "${RADEON_DPM_PERF_LEVEL_ON_AC}!auto!low!high")
    yad_opts+=( --field="RADEON DPM PERF LEVEL ON BAT:cb" "${RADEON_DPM_PERF_LEVEL_ON_BAT}!auto!low!high")
	yad "${yad_opts[@]}" > $Graphics &
	
	#  - name: Network
	yad_opts=()
	yad_opts=(--plug=$KEY --tabnum=5 --borders=10
	--image=/usr/share/icons/hicolor/128x128/apps/mbcc.png
	--columns=1 --form)
    yad_opts+=( --field="WIFI PWR ON AC:CHK" "${WIFI_PWR_ON_AC}")
    yad_opts+=( --field="WIFI PWR ON BAT:CHK" "${WIFI_PWR_ON_BAT}")
    yad_opts+=( --field="WOL DISABLE:CHK" "${WOL_DISABLE}")
	yad "${yad_opts[@]}" > $Network &
	
	#  - name: PCIe
	yad_opts=()
	yad_opts=(--plug=$KEY --tabnum=6 --borders=10
	--image=/usr/share/icons/hicolor/128x128/apps/mbcc.png
	--columns=1 --form)
    yad_opts+=( --field="PCIE ASPM ON AC:cb" "${PCIE_ASPM_ON_AC}!default!performance!powersave!powersupersave")
    yad_opts+=( --field="PCIE ASPM ON BAT:cb" "${PCIE_ASPM_ON_BAT}!default!performance!powersave!powersupersave")
    yad_opts+=( --field="RUNTIME PM ON AC:CHK" "${RUNTIME_PM_ON_AC}")
    yad_opts+=( --field="RUNTIME PM ON BAT:CHK" "${RUNTIME_PM_ON_BAT}")
    yad_opts+=( --field="RUNTIME PM DENYLIST!gtk-yes:FBTN" "bash -c _RUNTIME_PM_DENYLIST")
    yad_opts+=( --field="RUNTIME PM DRIVER DENYLIST" "$RUNTIME_PM_DRIVER_DENYLIST")
    [[ "$__tlp_version_" != "1.3" ]] && yad_opts+=(--field="RUNTIME PM ENABLE" "$RUNTIME_PM_ENABLE")
	[[ "$__tlp_version_" != "1.3" ]] && yad_opts+=(--field="RUNTIME PM DISABLE" "$RUNTIME_PM_DISABLE")
	yad "${yad_opts[@]}" > $PCIe &
	
	#  - name: Processor
	yad_opts=()
	yad_opts=(--plug=$KEY --tabnum=7 --borders=10
	--image=/usr/share/icons/hicolor/128x128/apps/mbcc.png
	--columns=1 --form)
	if [[ "$__tlp_version_" = "1.7" ]] || [[ "$__tlp_version_" = "1.6" ]];then
		yad_opts+=(--field="CPU DRIVER OPMODE ON AC:cb" "${CPU_DRIVER_OPMODE_ON_AC}!active!passive!guided")
		yad_opts+=(--field="CPU DRIVER OPMODE ON BAT:cb" "${CPU_DRIVER_OPMODE_ON_BAT}!active!passive!guided")
	fi
    yad_opts+=( --field="CPU SCALING GOVERNOR_ON AC:cb" "${CPU_SCALING_GOVERNOR_ON_AC}!ondemand!powersave!performance!conservative!schedutil")
    yad_opts+=( --field="CPU SCALING GOVERNOR_ON BAT:cb" "${CPU_SCALING_GOVERNOR_ON_BAT}!ondemand!powersave!performance!conservative!schedutil")
    yad_opts+=( --field="CPU SCALING MIN FREQ ON AC" "$CPU_SCALING_MIN_FREQ_ON_AC")
    yad_opts+=( --field="CPU SCALING MAX FREQ ON AC" "$CPU_SCALING_MAX_FREQ_ON_AC")
    yad_opts+=( --field="CPU SCALING MIN FREQ ON BAT" "$CPU_SCALING_MIN_FREQ_ON_BAT")
    yad_opts+=( --field="CPU SCALING MAX FREQ ON BAT" "$CPU_SCALING_MAX_FREQ_ON_BAT")
    yad_opts+=( --field="CPU ENERGY PERF POLICY ON AC:cb" "${CPU_ENERGY_PERF_POLICY_ON_AC}!default!performance!balance_performance!balance_power")
    yad_opts+=( --field="CPU ENERGY PERF POLICY ON BAT:cb" "${CPU_ENERGY_PERF_POLICY_ON_BAT}!default!performance!balance_performance!balance_power")
    yad_opts+=( --field="CPU MIN PERF ON AC::NUM" "$CPU_MIN_PERF_ON_AC"!0..100!1)
    yad_opts+=( --field="CPU MAX PERF ON AC::NUM" "$CPU_MAX_PERF_ON_AC"!0..100!1)
    yad_opts+=( --field="CPU MIN PERF ON BAT::NUM" "$CPU_MIN_PERF_ON_BAT"!0..100!1)
    yad_opts+=( --field="CPU MAX PERF ON BAT::NUM" "$CPU_MAX_PERF_ON_BAT"!0..100!1)
    yad_opts+=( --field="CPU BOOST ON AC:CHK" "${CPU_BOOST_ON_AC}")
    yad_opts+=( --field="CPU BOOST ON BAT:CHK" "${CPU_BOOST_ON_BAT}")
    if [[ "$__tlp_version_" != "1.3" ]];then
    	yad_opts+=(--field="CPU HWP DYN BOOST ON AC:CHK" "${CPU_HWP_DYN_BOOST_ON_AC}")
    	yad_opts+=(--field="CPU HWP DYN BOOST ON BAT:CHK" "${CPU_HWP_DYN_BOOST_ON_BAT}")
    fi
    if [[ "$__tlp_version_" = "1.5" ]] || [[ "$__tlp_version_" = "1.4" ]] || [[ "$__tlp_version_" = "1.3" ]];then
    	yad_opts+=(--field="SCHED POWERSAVE ON AC:CHK" "${SCHED_POWERSAVE_ON_AC}")
		yad_opts+=(--field="SCHED POWERSAVE ON BAT:CHK" "${SCHED_POWERSAVE_ON_BAT}")
	fi
    yad_opts+=(--field="NMI WATCHDOG:CHK" "${NMI_WATCHDOG}")
    [[ "$__tlp_version_" = "1.3" ]] && yad_opts+=(--field="PHC CONTROLS" "$PHC_CONTROLS")
    if [[ "$__tlp_version_" != "1.3" ]];then
    	yad_opts+=(--field="PLATFORM PROFILE ON AC:cb" "${PLATFORM_PROFILE_ON_AC}!performance!balanced!low-power")
		yad_opts+=(--field="PLATFORM PROFILE ON BAT:cb" "${PLATFORM_PROFILE_ON_BAT}!performance!balanced!low-power")
    	if [[ "$__tlp_version_" != "1.5" ]] && [[ "$__tlp_version_" != "1.4" ]];then	
    		yad_opts+=(--field="MEM SLEEP ON AC:cb" "${MEM_SLEEP_ON_AC}!s2idle!deep")
			yad_opts+=(--field="MEM SLEEP ON BAT:cb" "${MEM_SLEEP_ON_BAT}!s2idle!deep")
		fi
	fi
	...
	#  - name: Radio
	yad_opts=()
	yad_opts=(--plug=$KEY --tabnum=8 --borders=10
	--image=/usr/share/icons/hicolor/128x128/apps/mbcc.png
	--columns=1 --form)
    yad_opts+=( --field="RESTORE DEVICE STATE ON STARTUP:CHK" "${RESTORE_DEVICE_STATE_ON_STARTUP}")
	yad_opts+=( --field="DEVICES TO DISABLE ON STARTUP" "${DEVICES_TO_DISABLE_ON_STARTUP}")
    yad_opts+=( --field="DEVICES TO ENABLE ON STARTUP" "${DEVICES_TO_ENABLE_ON_STARTUP}")
    if [[ "$__tlp_version_" != "1.7" ]];then
    	yad_opts+=(--field="DEVICES TO DISABLE ON SHUTDOWN" "${DEVICES_TO_DISABLE_ON_SHUTDOWN}")
    	yad_opts+=(--field="DEVICES TO ENABLE ON SHUTDOWN" "${DEVICES_TO_ENABLE_ON_SHUTDOWN}")
    fi
    yad_opts+=( --field="DEVICES TO ENABLE ON AC" "${DEVICES_TO_ENABLE_ON_AC}")
    yad_opts+=( --field="DEVICES TO DISABLE ON BAT" "${DEVICES_TO_DISABLE_ON_BAT}")
    yad_opts+=( --field="DEVICES TO DISABLE ON BAT NOT IN USE" "${DEVICES_TO_DISABLE_ON_BAT_NOT_IN_USE}")
    yad_opts+=( --field="DEVICES TO DISABLE ON LAN CONNECT" "${DEVICES_TO_DISABLE_ON_LAN_CONNECT}")
    yad_opts+=( --field="DEVICES TO DISABLE ON WIFI CONNECT" "${DEVICES_TO_DISABLE_ON_WIFI_CONNECT}")
    yad_opts+=( --field="DEVICES TO DISABLE ON WWAN CONNECT" "${DEVICES_TO_DISABLE_ON_WWAN_CONNECT}")
    yad_opts+=( --field="DEVICES TO ENABLE ON LAN DISCONNECT" "${DEVICES_TO_ENABLE_ON_LAN_DISCONNECT}")
    yad_opts+=( --field="DEVICES TO ENABLE ON WIFI DISCONNECT" "${DEVICES_TO_ENABLE_ON_WIFI_DISCONNECT}")
    yad_opts+=( --field="DEVICES TO ENABLE ON WWAN DISCONNECT" "${DEVICES_TO_ENABLE_ON_WWAN_DISCONNECT}")
    yad_opts+=( --field="DEVICES TO ENABLE ON DOCK" "${DEVICES_TO_ENABLE_ON_DOCK}")
    yad_opts+=( --field="DEVICES TO DISABLE ON DOCK" "${DEVICES_TO_DISABLE_ON_DOCK}")
    yad_opts+=( --field="DEVICES TO ENABLE ON UNDOCK" "${DEVICES_TO_ENABLE_ON_UNDOCK}")
    yad_opts+=( --field="DEVICES TO DISABLE ON UNDOCK" "${DEVICES_TO_DISABLE_ON_UNDOCK}")
	yad "${yad_opts[@]}" > $Radio &

	#  - name: USB
	yad_opts=()
	yad_opts=(--plug=$KEY --tabnum=9 --borders=10
	--image=/usr/share/icons/hicolor/128x128/apps/mbcc.png
	--columns=1 --form)
    yad_opts+=( --field="USB AUTOSUSPEND:CHK" "${USB_AUTOSUSPEND}")
    yad_opts+=( --field="USB DENYLIST!gtk-yes:FBTN" "bash -c _USB_DENYLIST")
    [[ "$__tlp_version_" != "1.3" ]] && yad_opts+=( --field="USB EXCLUDE AUDIO:CHK" "${USB_EXCLUDE_AUDIO}")
    yad_opts+=( --field="USB EXCLUDE BTUSB:CHK" "${USB_EXCLUDE_BTUSB}")
    yad_opts+=( --field="USB EXCLUDE PHONE:CHK" "${USB_EXCLUDE_PHONE}")
    yad_opts+=( --field="USB EXCLUDE PRINTER:CHK" "${USB_EXCLUDE_PRINTER}")
    yad_opts+=( --field="USB EXCLUDE WWAN:CHK" "${USB_EXCLUDE_WWAN}")
    yad_opts+=( --field="USB ALLOWLIST!gtk-yes:FBTN" "bash -c _USB_ALLOWLIST")
    [[ "$__tlp_version_" != "1.7" ]] && yad_opts+=(--field="USB AUTOSUSPEND DISABLE ON SHUTDOWN:CHK" "${USB_AUTOSUSPEND_DISABLE_ON_SHUTDOWN}")
	yad "${yad_opts[@]}" > $USB &

	#  - name: ThinkPad_Battery
	yad_opts=()
	yad_opts=(--plug=$KEY --tabnum=10 --borders=10
	--image=/usr/share/icons/hicolor/128x128/apps/mbcc.png
	--columns=1 --form)
    yad_opts+=( --field="START CHARGE THRESH BAT0::NUM" "${__numeric_START_CHARGE_THRESH_0_}")
    yad_opts+=( --field="STOP CHARGE THRESH BAT0::NUM" "${__numeric_STOP_CHARGE_THRESH_0_}")
    yad_opts+=( --field="START CHARGE THRESH BAT1::NUM" "${__numeric_START_CHARGE_THRESH_1_}")
    yad_opts+=( --field="STOP CHARGE THRESH_BAT1::NUM" "${__numeric_STOP_CHARGE_THRESH_1_}")
    yad_opts+=( --field="RESTORE THRESHOLDS ON BAT:CHK" "${RESTORE_THRESHOLDS_ON_BAT}")
    yad_opts+=( --field="NATACPI ENABLE:CHK" "${NATACPI_ENABLE}")
    yad_opts+=( --field="TPACPI ENABLE:CHK" "${TPACPI_ENABLE}")
    yad_opts+=( --field="TPSMAPI ENABLE:CHK" "${TPSMAPI_ENABLE}")
	...
	yad --notebook --tab-pos="left" --key=$KEY \
    	...
    	--title="TLP" --image=/usr/share/DmDmDmdMdMdM/images/linux.png \
...
show_statistics(){
	if [ "$1" = "complate" ];then
		tlp_stat="$(yad --center --width=300 --image=keys.png --title 'Sudo-Password' --button='Ok:0' --button='Cancel:1'  --entry --hide-text --text 'Please enter your password' | sudo -S $(which tlp-stat))"
	else
