...
add_keybinds "thunar" || continue

__thunar_wall_plug="$(locate thunar | grep "plugin.so" | grep "wall" | grep -v "\.backup" || :)"
[ -f "${__thunar_wall_plug}" ] && [ ! -f "${__thunar_wall_plug}.backup" ] && my-superuser mv "${__thunar_wall_plug}" "${__thunar_wall_plug}.backup"
