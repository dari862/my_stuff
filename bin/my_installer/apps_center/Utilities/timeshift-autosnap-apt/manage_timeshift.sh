...
list_snapshots() {
    ...
    my-superuser timeshift --list-snapshots
}
...
create_snapshot() {
    ...
    if [ -z "$COMMENT" ] && [ -z "$TAG" ]; then
        ...
        my-superuser timeshift --create
    elif [ -z "$TAG" ]; then
        printf "%b\n" "${CYAN}Creating snapshot with no tag...${RC}"
        my-superuser timeshift --create --comments "$COMMENT"
    else
        printf "%b\n" "${CYAN}Creating snapshot with tag: $TAG...${RC}"
        my-superuser timeshift --create --comments "$COMMENT" --tags "$TAG"
    fi
...
restore_snapshot() {
    ...
    if [ "$SKIP_GRUB" = "y" ] || [ "$SKIP_GRUB" = "Y" ]; then
        my-superuser timeshift --restore --snapshot "$SNAPSHOT" --target-device "$TARGET_DEVICE" --skip-grub --yes
    else
        printf "%b\n" "${CYAN}Enter GRUB device (e.g., /dev/sda): ${RC}"
        read -r GRUB_DEVICE
        my-superuser timeshift --restore --snapshot "$SNAPSHOT" --target-device "$TARGET_DEVICE" --grub-device "$GRUB_DEVICE" --yes
    fi
    # shellcheck disable=SC2181
    if [ $? -eq 0 ]; then
...
delete_snapshot() {
    ...
    my-superuser timeshift --delete --snapshot "$SNAPSHOT" --yes
    # shellcheck disable=SC2181
    if [ $? -eq 0 ]; then
...
delete_all_snapshots() {
    ...
    if [ "$CONFIRMATION" = "y" ] || [ "$CONFIRMATION" = "Y" ]; then
        ...
        my-superuser timeshift --delete-all --yes
        # shellcheck disable=SC2181
        if [ $? -eq 0 ]; then
