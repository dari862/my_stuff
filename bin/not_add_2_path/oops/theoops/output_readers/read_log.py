...
def _skip_old_lines(log_file):
    size = os.path.getsize(os.environ['THEOOPS_OUTPUT_LOG'])
...
def get_output(script):
    ...
    """
    if 'THEOOPS_OUTPUT_LOG' not in os.environ:
    ...
    if const.USER_COMMAND_MARK not in os.environ.get('PS1', ''):
        logs.warn(
            ...
            "that PS1 is not changed after The Oops alias initialization")
        return None

    try:
        with logs.debug_time(u'Read output from log'):
            fd = os.open(os.environ['THEOOPS_OUTPUT_LOG'], os.O_RDONLY)
